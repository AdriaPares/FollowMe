...
def Add_Dash(server):
    ...
    prepared_query_genre = session.prepare("select timestamp, follower_count from twitter_day "
                                           "where streamer=?");
    prepared_query_language = session.prepare("select timestamp, follower_count from twitch_day "
                                              "where streamer=?");
    prepared_query_console = session.prepare("select timestamp, follower_count from youtube_hour "
                                             "where streamer=?");
