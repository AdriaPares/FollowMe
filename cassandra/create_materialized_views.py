...
def get_views(day: str = dt.datetime.today().strftime('%Y-%m-%d'),
    ...
    for table_to_view in tables_to_view:
        views.append("create MATERIALIZED VIEW " + table_to_view + "_view as select * from " + table_to_view
                     + " where streamer is not null and timestamp > '" + day + "primary key (timestamp, streamer) ;")

    # Unified day requires the last month for plotting in Dash
    views.append("create MATERIALIZED VIEW unified_day_view as select * from unified_day where streamer is not null "
                 "and timestamp > '" + last_month + "primary key (timestamp, streamer) ;")
    return views
